...
def reformat(choice):
    ...
    if is_proffie:
        ...
        for old_cfx_file in os.listdir(proffie_path):
            if 'pwroff2' in old_cfx_file:
                os.rename(f'{proffie_path}{old_cfx_file}',
                          f'{proffie_path}/in/in{str(max_pwr_off+1).zfill(2)}.wav')
            if 'poweronf' in old_cfx_file:
                os.rename(f'{proffie_path}{old_cfx_file}',
                          f'{proffie_path}/out/out{str(max_pwr_on+1).zfill(2)}.wav')
    ...
    if is_xeno:
        ...
        for xeno_file in os.listdir(xeno_path):
            ...
            elif 'enddrag' in xeno_file:
                os.rename(f'{xeno_path}{xeno_file}',
                          f'{xeno_path}enddrag ({solo}{xeno_file[xeno_file.index("g") + 1:xeno_file.index(".")]}).wav')
